...
export class WalletProvider {
    private currentChain: SupportedChain = "seiTestnet";
    chains: Record<string, Chain> = { seiTestnet: viemChains.seiTestnet };
...
const genChainsFromRuntime = (
    ...
    const chainNames = ["sei", "seiTestnet"];
    const chains = {};
    ...
    const mainnet_rpcurl = runtime.getSetting("SEI_PROVIDER_URL");
    if (mainnet_rpcurl) {
        const chain = WalletProvider.genChainFromName("sei", mainnet_rpcurl);
        chains["sei"] = chain;
    }

    const testnet_rpcurl = runtime.getSetting("SEI_TESTNET_PROVIDER_URL");
    if (testnet_rpcurl) {
        const chain = WalletProvider.genChainFromName("seiTestnet", testnet_rpcurl);
        chains["seiTestnet"] = chain;
    }
...
export const seiWalletProvider: Provider = {
    async get(
        runtime: IAgentRuntime,
        _message: Memory,
        _state?: State
        ...
        try {
            ...
            return `sei chain Wallet Address: ${address}\nBalance: ${balance} ${chain.nativeCurrency.symbol}\nChain ID: ${chain.id}, Name: ${chain.name}`;
        } catch (error) {
            console.error("Error in sei chain wallet provider:", error);
            return null;
        }
    },
};
